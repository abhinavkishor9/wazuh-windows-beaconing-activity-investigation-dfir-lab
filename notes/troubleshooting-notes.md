...
## Issue 5 – Wazuh Search Returned Too Many Results
...
Windows and Sysmon generate significant amounts of endpoint telemetry.
...
Also restrict the Wazuh time range to the period in which the test was performed.

---

## Issue 6 – OwningProcess Did Not Match the Expected PID
...
## Issue 7 – Connection Intervals Were Not Exactly 30 Seconds
...
## Issue 8 – Beacon-Like Pattern Could Be Misclassified
...
## Issue 9 – Listener Remained Active After Testing
...
## Issue 10 – Baseline Contained Unrelated Connections

### Problem

The initial network baseline showed many existing connections unrelated to the lab.

### Cause

Normal Windows activity and background applications generate network connections continuously.

### Resolution

Record the baseline before starting the test and focus the investigation on:
...
Do not treat unrelated background traffic as part of the beaconing test.

---

# Lessons Learned

- A short-lived connection may not be visible in live connection tables.
- Sysmon Event ID 3 is useful for historical network connection analysis.
- Process IDs are critical for connecting network activity to the responsible process.
- Wazuh field names should be verified against actual indexed events.
- Periodic communication is a detection signal, not automatic proof of C2.
- Legitimate software can generate regular network connections.
- The destination, process, user, and surrounding activity must be considered together.
- Controlled lab activity should always be cleaned up after the investigation.
